...
const globalForHomeSchema = globalThis as unknown as {
  homeSchemaReady?: Promise<void>;
};

export function ensureHomeSchema(): Promise<void> {
  if (!globalForHomeSchema.homeSchemaReady) {
    globalForHomeSchema.homeSchemaReady = (async () => {
      const pool = getPool();
      await pool.query(`
      ...
      globalForHomeSchema.homeSchemaReady = undefined;
      throw error;
    });
  }

  return globalForHomeSchema.homeSchemaReady;
}
